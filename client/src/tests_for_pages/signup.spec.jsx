...
import SignupPage, { getServerSideProps } from '@/pages/signup';
...
describe('renders', () => {
  ...
  test('with username', async () => {
    ...
    const response = await SignupPage(data);
    render(response);
...
describe('test login', () => {
  ...
  test('error', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Sign up' })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Username taken' })).toBeInTheDocument();
